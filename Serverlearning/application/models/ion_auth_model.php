		...
		if (!empty($groups))
		{
			...
			foreach ($groups as $group)
			{
                        ...
                        $authors = array(
                                    ...
                                    'author_id'=>$id,
                                    'company_id'=>1,
                                    'auth_name'=>'User Defined Auth'
                        );
                        $this->db->insert('dsi_authors', $authors); 
                        $is_finished = FALSE;
                        
                        if ($user_group_id == 2) {
                            $view_id = 1; // 'creation_view';
                            $tier_no = 1;
                            $previllage = 1;
                            
                        } else if ($user_group_id == 3) {
                            $view_id = 2; //'verification_view';
                            $tier_no = 2;
                            $previllage = 1;
                        } else if ($user_group_id == 4) {
                            $view_id = 3; //'authorization_view'; 
                            $tier_no = 3;
                            $previllage = 1;
                        } else if ($user_group_id == 5) {
                            $view_id = 4; //'monitoring_view';
                            $tier_no = 0;
                            $previllage = 0;
                        } else {
                            $view_id = 5; //'execution_view';
                            $tier_no = 4;
                            $previllage = 1;
                            $is_finished = TRUE;
                        }
                        ...
                        $work_flow = array(
                                    ...
                                    'view_id'=>$view_id,
                                    'tier_no'=>$tier_no,
                                    'access_right'=>$previllage,
                                    'is_finished'=>$is_finished,
                                    'is_active'=>TRUE
                        );
                        $this->db->insert('dsi_works_flows', $work_flow);
		}
